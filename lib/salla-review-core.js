...
function componentRefsThroughIncludes(themeRoot, startFile, factsByFile) {
    ...
    while (stack.length) {
        ...
        if (!entry) continue;
        for (const r of entry.facts.componentRefs) refs.add(r);
...
function checkRequiredHooksFromFacts(themeRoot, factsByFile, issues, filters, manifest) {
 ...
 */
function checkRequiredComponentsFromFacts(themeRoot, factsByFile, issues, manifest) {
    for (const [rel, spec] of Object.entries(REQUIRED_COMPONENTS)) {
        ...
        const present = entry ? entry.facts.sallaComponents : null;
...
function runCrossChecks(state) {
    ...
    if (manifest.exists) {
        ...
        if (opts.requiredHooks !== false) checkRequiredHooksFromFacts(projectRoot, factsByFile, issues, filters, manifest);
        if (opts.requiredComponents !== false) checkRequiredComponentsFromFacts(projectRoot, factsByFile, issues, manifest);
