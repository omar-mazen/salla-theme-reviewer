...
function checkThemeSize(themeRoot, issues, filters, manifest, size) {
...
const TWIG_TEMPLATE_REF_RE =
    /\{%-?\s*(?:include|embed|import|from|use)\s+['"]([^'"]+)['"]|\binclude\s*\(\s*['"]([^'"]+)['"]/g;
    ...
    for (const [rel, comps] of Object.entries(REQUIRED_COMPONENTS)) {
        ...
        for (const comp of comps) {
            ...
            issues.push({
                ...
                line: 1,
                desc: entry
                    ? `مكوّن مطلوب غير مضاف في src/views/${rel}: <${comp}> — يجب أن يرد داخل هذا الملف مباشرةً (وجوده في ملف آخر أو داخل ملف مُضمَّن بـ include لا يكفي) — راجع ثيم رائد كمرجع`
                    : `الملف src/views/${rel} غير موجود، وهو مطلوب ويجب أن يحتوي المكوّن <${comp}> مباشرةً — راجع ثيم رائد كمرجع`,
...
function finishFullScan(state, walk) {
    ...
    if (state.opts.sizeCheck !== false && state.manifest.exists) {
        checkThemeSize(state.projectRoot, state.sizeIssues, state.filters, state.manifest, walk.size);
    }
