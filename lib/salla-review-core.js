...
const REQUIRED_COMPONENTS = {
    ...
    "pages/cart.twig": {
        fileRequired: true, severity: "error",
        components: ["salla-cart-coupons", "salla-cart-item-offers", "salla-cart-summary-card", "salla-order-totals-card", "salla-loyalty-panel", "salla-tiered-offer"],
    },
...
function checkProductsSliderSource(fileAbs, ctx, issues) {
    ...
    for (const opener of PRODUCT_SOURCE_TAGS) {
        ...
        while ((idx = src.indexOf(opener, idx)) !== -1) {
            ...
            } else if (hasSourceValue && !/json_encode/.test(tag)) {
                push("warning", `مكوّن ${name}: مرّر source-value عبر json_encode — source-value="{{ ...source_value|json_encode }}". ${REF}`);
            }
            // A component wired to a section must receive its source and limit
            if (/\bsource\s*=/.test(tag) && !/\blimit\s*=/.test(tag)) {
                push("warning", `مكوّن ${name} بلا خاصية limit — قائمة الاعتماد لدى سلة تشترط تمرير source وlimit وsource-value من بيانات القسم. ${REF}`);
            }
            idx = j + 1;
        }
    }
...
function collectFileFacts(fileAbs, opts) {
    ...
    const collectSettingsUsed = (src, lineStarts) => {
        ...
        while ((m = SETTINGS_GET_RE.exec(src))) {
            if (!facts.settingsUsed.has(m[1])) facts.settingsUsed.set(m[1], lineAtIndex(lineStarts, m.index));
        }
    };
...
function resolveTemplateRef(themeRoot, fromFile, rel, factsByFile) {
    ...
    for (const abs of [
        path.join(themeRoot, "src", "views", ...parts),
        path.join(path.dirname(fromFile), ...parts),
    ]) {
...
function checkTemplateRefs(themeRoot, factsByFile, issues) {
    ...
    for (const [file, entry] of factsByFile) {
        ...
        for (const ref of entry.facts.includes) {
            const parts = String(ref).replace(/\\/g, "/").split("/").filter((s) => s && s !== ".");
            if (!parts.length) continue;
            const candidates = [path.join(themeRoot, "src", "views", ...parts), path.join(path.dirname(file), ...parts)];
