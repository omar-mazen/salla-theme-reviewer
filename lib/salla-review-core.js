...
function checkHardcodedColors(fileAbs, kind, ctx, issues) {
    ...
    if (kind === "twig") {
        ...
        let src = maskRegions(ctx.twigCommentMasked, /<svg\b[\s\S]*?<\/svg\s*>/gi);
...
function checkRequiredComponentsFromFacts(themeRoot, factsByFile, issues, manifest) {
    ...
    for (const [rel, comps] of Object.entries(REQUIRED_COMPONENTS)) {
        ...
        for (const comp of comps) {
            ...
            const anchor = exists ? abs : manifest.path;
            const lines = exists ? factsByFile.get(abs).lines : manifest.lines;
            issues.push({
                type: "Twilight Components",
                file: anchor,
                line: 1,
