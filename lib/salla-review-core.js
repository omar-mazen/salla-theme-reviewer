...
function checkHardcodedColors(fileAbs, kind, issues) {
    ...
    for (let i = 0; i < scanLines.length; i++) {
        ...
        while ((m = TAILWIND_COLOR_RE.exec(line))) found.push(m[0]);
...
const CSS_VAR_PLATFORM_PREFIXES = ["tw-", "swiper-", "salla-", "s-", "color-", "font-", "anim"];
...
const CSS_VAR_USE_RE = /var\(\s*--([A-Za-z0-9_-]+)/g;
