...
/** One in-flight fetch per cache file — themes checked at the same time share it instead of each hitting the registry */
const inFlight = new Map();

/** All version lists for the three packages — from the cache or the network (the three fetches run in parallel) */
async function getVersionLists(cacheFile) {
    ...
    const p = (async () => {
        const lists = await Promise.all(PACKAGES.map((pkg) => fetchVersionList(pkg)));
        const data = {};
        PACKAGES.forEach((pkg, i) => { data[pkg] = lists[i]; });
...
async function checkTwilightVersions(projectRoot, opts = {}) {
    ...
    for (const name of used) {
        ...
        issues.push({
            ...
            desc: `إصدار ${name} قديم: الحالي ${current} والأحدث ${latest} — الفارق ${behind} إصدارًا ويتجاوز الحد المسموح (${MAX_VERSIONS_BEHIND} إصدارات). حدّث الحزمة، ثم نفّذ التثبيت (pnpm install / npm install) ليتحدّث ملف القفل معها وإلا فشل التثبيت في CI، ثم npm run prod وارفع الأصول وملف القفل`,
            lines: pkgLines,
        });
    }
    return issues;
